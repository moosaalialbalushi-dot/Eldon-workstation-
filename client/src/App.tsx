...
import { 
  ...
  CheckSquare, HelpCircle, Briefcase
} from 'lucide-react';
...
export default function App() {
    ...
    filesArray.forEach(file => {
      ...
      reader.onload = (event) => {
        const newFile = {
          ...
          content: isText ? event.target.result : `[Binary File metadata: ${file.name}. Vector extraction required via backend.]`
        };
  ...
  const getFileContextString = () => {
    ...
    const fileContents = skillsFiles.map(f => `--- FILE: ${f.name} ---\n${f.content ? f.content.substring(0, 3000) : 'Binary Content'}`).join("\n\n");
  ...
  const callGeminiAPI = async (promptText) => {
    ...
    if (!apiKey) {
      return "⚠️ System Alert: Gemini API Key is missing. Please go to the 'Settings & APIs' tab and paste your Gemini key.";
    }
  ...
  const handleAddCustomAI = (e) => {
    ...
    setAvailableAIs([...availableAIs, { id: newId, name: newAiName, theme: 'bg-gray-400 text-gray-900' }]);
    setNewAiName('');
  };
  ...
  useEffect(() => {
    const generateReplies = async () => {
      ...
      if (lastMsg.role === 'ai' && lastMsg.ai !== 'system' && !pipelineState.active) {
        ...
        try {
          const prompt = `Generate 3 short follow-up questions for this: "${lastMsg.content.substring(0, 500)}". Output raw JSON array of 3 strings.`;
  ...
  const executeMasterPipeline = async (userMessage) => {
    ...
    try {
      ...
      setMessages(prev => [...prev, { role: 'ai', content: "Pipeline interrupted.", ai: 'master-skill' }]);
    }
  ...
  const handleSendMessage = async () => {
    if (!input.trim()) return;
    const newUserMsg = { role: 'user', content: input, ai: selectedAI };
      ...
      setTimeout(() => {
        ...
        if (selectedAI === 'manus') {
          connectorInfo = "\n\n**Manus Operations Performed:**\n" +
            ...
            (useGithub ? `✅ Code staged and pushed to GitHub.\n` : "") +
            (useVercel ? `✅ Triggered deployment via Vercel.\n` : "") +
            (useSupabase ? `✅ Database schema synced with Supabase.\n` : "") +
           ...
           connectorInfo = "\n\n**NotebookLM Media Triggered:**\n" +
            (requestSlides ? "✅ Slide deck rendering.\n" : "") +
            (requestVideo ? "✅ Video timeline generating.\n" : "") +
        ...
        setMessages(prev => [...prev, { 
          role: 'ai', 
          content: `[Simulation] Mock response from **${variantName}**.\n\nRoutes via backend to API endpoint.${connectorInfo}${fileStatus}`, 
          ai: selectedAI,
          variant: variantName
        }]);
        setIsLoading(false);
            ...
            <div className="h-16 border-b border-gray-800 flex items-center justify-between px-4 md:px-6 bg-[#111111]/80 backdrop-blur-md sticky top-0 z-10 shadow-sm pl-16 md:pl-6">
              ...
              <div className="flex space-x-2">
                <button 
                  ...
                  disabled={messages.length <= 1 || isExtractingActions}
                  className="px-3 py-1.5 text-xs font-medium bg-[#1a1a1a] hover:bg-gray-800 text-amber-500 rounded-md transition-colors flex items-center space-x-1 border border-gray-700 disabled:opacity-50"
                  title="✨ Extract action items"
                >
                  {isExtractingActions ? <CircleDashed size={14} className="animate-spin" /> : <CheckSquare size={14} />}
                  <span className="hidden sm:inline">Tasks</span>
                </button>
                <button 
                  onClick={handleSummarizeChat} 
                  disabled={messages.length <= 1 || isSummarizingChat}
                  className="px-3 py-1.5 text-xs font-medium bg-[#1a1a1a] hover:bg-gray-800 text-amber-500 rounded-md transition-colors flex items-center space-x-1 border border-gray-700 disabled:opacity-50"
                  title="✨ Let Gemini summarize this chat"
                >
                  {isSummarizingChat ? <CircleDashed size={14} className="animate-spin" /> : <Sparkles size={14} />}
                    ...
                    <div className={`flex flex-col space-y-3 ${msg.role === 'user' ? 'items-end' : 'items-start'} max-w-full overflow-hidden`}>
                      <div className={`p-4 rounded-xl shadow-sm overflow-x-auto ${
                        ...
                        {msg.role === 'ai' && msg.ai !== 'system' && (
                          <div className="mt-4 pt-3 border-t border-gray-800/50 text-xs text-amber-600/70 flex items-center justify-between font-medium">
                            ...
                            <button 
                              onClick={() => handleSimplifyMessage(idx, msg.content)}
                              disabled={simplifyingIndex === idx}
                              className="flex items-center space-x-1 hover:text-amber-400 transition-colors bg-amber-500/10 px-2 py-1 rounded disabled:opacity-50"
                              title="✨ Explain Like I'm 5"
                            >
                              {simplifyingIndex === idx ? <CircleDashed size={12} className="animate-spin" /> : <Sparkles size={12} />}
                              <span>ELI5</span>
                            </button>
                          </div>
                        )}
              ...
              <div className="max-w-4xl w-full mx-auto relative flex items-end bg-[#111111] rounded-xl border border-gray-700 shadow-xl focus-within:ring-1 focus-within:ring-amber-500 overflow-hidden transition-all">
                ...
                <div className="flex flex-shrink-0 items-center">
                  <button
                    ...
                    disabled={!input.trim() || isRewriting || isLoading}
                    title="✨ Polish Tone to Professional"
                  ...
                  >
                    {isRewriting ? <div className="w-5 h-5 border-2 border-amber-500 border-t-transparent rounded-full animate-spin"></div> : <Briefcase size={18} />}
                  </button>
                  <button
          ...
          <div className="flex-1 overflow-y-auto p-6 md:p-10 pt-20 md:pt-10">
            <div className="max-w-4xl mx-auto">
                    ...
                    <div className="flex items-center justify-between">
                      <div className="flex items-center space-x-3 overflow-hidden">
                        ...
                        <div className="truncate">
                          ...
                          <p className="text-xs text-gray-500">
                            ...
                            <span className={`ml-2 px-1.5 py-0.5 rounded text-[10px] ${file.content.includes('[Binary') ? 'bg-red-500/20 text-red-400' : 'bg-green-500/20 text-green-400'}`}>
                              {file.content.includes('[Binary') ? 'Binary' : 'Text Parsed'}
                            </span>
                          </p>
                        </div>
                      </div>
                      <div className="flex items-center space-x-1 flex-shrink-0 pl-2">
                        <button 
                          ...
                          disabled={summarizingFileId === file.id || file.content.includes('[Binary')}
                    ...
                    {fileSummaries[file.id] && (
                      <div className="mt-3 p-3 bg-[#1a1a1a] border-l-2 border-amber-500 rounded-r-md text-sm text-gray-300">
                        <p className="flex items-start space-x-2">
                          ...
                          <span>{fileSummaries[file.id]}</span>
                        </p>
                      </div>
                    )}
                  </div>
              ...
              <div className="mt-8 border-t border-gray-800 pt-8 pb-10">
                <div className="flex flex-col sm:flex-row sm:items-center justify-between mb-4 space-y-3 sm:space-y-0">
                  ...
                  <div className="flex flex-col sm:flex-row space-y-2 sm:space-y-0 sm:space-x-2 w-full sm:w-auto">
                    <button 
                      ...
                      disabled={isGeneratingQuiz || skillsFiles.length === 0}
                      className="px-4 py-2 bg-[#1a1a1a] text-amber-500 hover:bg-gray-800 border border-gray-700 rounded-lg text-sm font-semibold transition-colors flex items-center justify-center space-x-2 disabled:opacity-50"
                    >
                      {isGeneratingQuiz ? <div className="w-4 h-4 border-2 border-amber-500 border-t-transparent rounded-full animate-spin"></div> : <HelpCircle size={16} />}
                      <span>✨ Gen Quiz</span>
                    </button>
                    <button 
                      onClick={handleDraftMasterReport}
                      disabled={isDraftingReport || skillsFiles.length === 0}
                      className="px-4 py-2 bg-[#1a1a1a] text-amber-500 hover:bg-gray-800 border border-gray-700 rounded-lg text-sm font-semibold transition-colors flex items-center justify-center space-x-2 disabled:opacity-50"
                    >
                      {isDraftingReport ? <div className="w-4 h-4 border-2 border-amber-500 border-t-transparent rounded-full animate-spin"></div> : <FileText size={16} />}
                    ...
                    >
                      {isGeneratingPlan ? <div className="w-4 h-4 border-2 border-amber-500 border-t-transparent rounded-full animate-spin"></div> : <Sparkles size={16} />}
                ...
                {quizContent && (
                  <div className="p-4 md:p-5 bg-gradient-to-br from-[#1a1a1a] to-[#111] border border-amber-500/20 rounded-xl shadow-inner mt-4">
                    <div className="text-gray-300 text-sm leading-relaxed whitespace-pre-wrap font-medium markdown-body">
                      {quizContent}
                    </div>
                  </div>
                )}
                {!studyPlan && !isGeneratingPlan && !quizContent && (
           ...
           <div className="flex-1 overflow-y-auto p-6 md:p-10 pt-20 md:pt-10">
            <div className="max-w-3xl mx-auto space-y-8">
              <div>
                ...
                <p className="text-gray-400 text-sm md:text-base">Configure your external connectors for orchestration. Keys are maintained locally before backend transit.</p>
              </div>
              ...
              <div className="bg-[#111111] border border-gray-800 rounded-xl p-5 md:p-6 mb-10 shadow-lg">
                ...
                <div className="space-y-4 mb-8">
                  <div>
                    <label className="block text-[10px] md:text-xs font-bold text-amber-500 uppercase tracking-wider mb-1">Google Gemini API Key (Required)</label>
                    <input 
                      type="password" 
                      value={geminiKey}
                      onChange={(e) => setGeminiKey(e.target.value)}
                      placeholder="AIzaSy..." 
                      className="w-full bg-[#1a1a1a] border border-amber-500/50 rounded-lg px-3 py-2 text-sm text-gray-300 focus:border-amber-500 focus:outline-none shadow-[0_0_10px_rgba(251,191,36,0.1)]" 
                    />
                    <p className="text-[10px] text-gray-500 mt-1">This key is saved securely in your browser's local storage.</p>
                  </div>
                  {availableAIs.filter(ai => ai.id !== 'master-skill' && ai.id !== 'gemini' && ai.id !== 'manus' && ai.id !== 'notebooklm').map(ai => (
                    <div key={ai.id}>
                      <label className="block text-[10px] md:text-xs font-bold text-gray-400 uppercase tracking-wider mb-1">{ai.name} API Key</label>
                      <input type="password" placeholder="Coming soon..." disabled className="w-full bg-[#1a1a1a] border border-gray-800 rounded-lg px-3 py-2 text-sm text-gray-500 cursor-not-allowed" />
                    </div>
                  ))}
                  
                  <div className="pt-2 flex items-center justify-between">
                    <span className={`text-sm font-medium transition-opacity duration-300 flex items-center ${keysSaved ? 'text-green-500 opacity-100' : 'opacity-0'}`}>
                      <Check size={16} className="mr-1" /> API Keys Saved Locally
                    </span>
                    <button 
                      onClick={handleSaveApiKeys}
                      disabled={!geminiKey}
                      className="px-6 py-2 bg-amber-500 text-gray-950 hover:bg-amber-400 rounded-lg font-bold transition-colors shadow-[0_0_10px_rgba(251,191,36,0.2)] disabled:opacity-50"
                    >
                      Save AI Keys
                    </button>
                  </div>
...
function PipelineStep({ number, name, active, completed }) {
  return (
